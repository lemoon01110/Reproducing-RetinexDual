...
0.934. PSNR matches to 0.034 dB under the same outputs, so the model is almost
...
Usage:
  python scripts/ssim_protocol_probe.py --repo ~/RetinexDual --data <testing_set> --limit 40
"""
import argparse
import os
...
def main():
    ...
    for k, name in enumerate(names):
        ...
        for key, val in protocols(out, gt).items():
    ...
    for key, vals in sorted(acc.items(), key=lambda kv: -st.mean(kv[1])):
        m = st.mean(vals)
        print(f"| `{key}` | {m:.5f} | {m - args.target:+.5f} |")
    ...
    if abs(best - args.target) < 0.002:
        print(f"Closest: `{best_key}` at {best:.5f}, within {abs(best - args.target):.5f} of "
              f"{args.target}.")
        print("That is close enough to explain the gap as a metric convention rather than a")
        print("difference in the model outputs. Still a hypothesis, not a confirmation of what")
        print("the authors ran.")
    else:
